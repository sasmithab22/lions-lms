...
async def upload_submission(
    ...
    try:
        date = str(datetime.now().date())
        folder = os.path.join("/tmp", "students", student, "submissions", date)
        os.makedirs(folder, exist_ok=True)
        filename = f"{title}_{file.filename}"
        filepath = os.path.join(folder, filename)
        with open(filepath, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
        return {"success": True, "filename": filename, "date": date}
    except Exception as e:
...
def student_submissions(student: str):
    base = os.path.join(STORAGE_DIR, "students", student, "submissions")
    result = []
...
def all_submissions():
    result = []
    base = os.path.join("/tmp", "students")
    if not os.path.exists(base):
        return []
    for student in os.listdir(base):
        student_path = os.path.join(base, student, "submissions")
...
def download_submission(student: str, date: str, filename: str):
    path = os.path.join("/tmp", "students", student, "submissions", date, filename)
...
def save_attendance(data: dict):
    ...
    date = data["date"]
    status = data["status"]
    file = os.path.join(STORAGE_DIR, "students", student, "attendance", "attendance.json")
    attendance = {}
...
def save_marks(data: dict):
    ...
    mark = data["mark"]
    file = os.path.join(STORAGE_DIR, "students", student, "marks", "marks.json")
    marks = {}
    if os.path.exists(file):
        with open(file, "r") as f:
            marks = json.load(f)
...
def school_analytics():
    base = os.path.join(STORAGE_DIR, "students")
    if not os.path.exists(base):
    ...
    for student in STUDENT_USERS.keys():
        ...
        student_data = {
            "student": student,
            "subject": STUDENT_USERS[student].get("subject", "-"),
            "tests": 0,
            "submissions": 0,
            "attendance": "-",
            "marks": "-"
        ...
        try:
            ...
            if os.path.exists(sub_path):
                for d in os.listdir(sub_path):
                    date_folder = os.path.join(sub_path, d)
                    if os.path.isdir(date_folder):
                        count += len([f for f in os.listdir(date_folder) if not f.startswith(".")])
    ...
    if attendance_count > 0:
        overall_attendance = round(attendance_present / attendance_count * 100, 1)

    avg_marks = 0
    if marks_count > 0:
        avg_marks = round(marks_total / marks_count, 1)
...
def student_report():
    ...
    for username, udata in STUDENT_USERS.items():
        ...
        marks_file = os.path.join(STORAGE_DIR, "students", username, "marks", "marks.json")
        if os.path.exists(marks_file):
            with open(marks_file) as f:
                marks = json.load(f)
            values = []
        ...
        att_file = os.path.join(STORAGE_DIR, "students", username, "attendance", "attendance.json")
        if os.path.exists(att_file):
            with open(att_file) as f:
                att = json.load(f)
            days = list(att.values())
            if len(days) > 0:
                present = len([d for d in days if d == "Present"])
                row["attendance_pct"] = round(present / len(days) * 100, 1)
...
def date_report(date: str):
    ...
    for username, data in STUDENT_USERS.items():
        ...
        attendance_file = os.path.join(STORAGE_DIR, "students", username, "attendance", "attendance.json")
...
async def upload_material(file: UploadFile = File(...)):
    folder = os.path.join(STORAGE_DIR, "materials")
...
@app.get("/api/reports")
def get_reports():
    base = os.path.join(STORAGE_DIR, "reports")
    if not os.path.exists(base):
        return []
    result = []
...
async def upload_report(file: UploadFile = File(...)):
    date = str(datetime.now().date())
    folder = os.path.join(STORAGE_DIR, "reports", date)
    filepath = os.path.join(folder, file.filename)
    with open(filepath, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
    return {"success": True}
...
async def upload_gallery(file: UploadFile = File(...)):
    date = str(datetime.now().date())
    folder = os.path.join(STORAGE_DIR, "gallery", date)
    filepath = os.path.join(folder, file.filename)
    with open(filepath, "wb") as buffer:
        shutil.copyfileobj(file.file, buffer)
    return {"success": True}
...
def gallery():
    result = []
    base = os.path.join(STORAGE_DIR, "gallery")
    if not os.path.exists(base):
        return []
    for date in os.listdir(base):
        date_folder = os.path.join(base, date)
        ...
        try:
            for file in os.listdir(date_folder):
                result.append({
                    "date": date,
                    "file": file,
                    "url": f"/storage/gallery/{date}/{file}"
                })
        except:
            continue
    return result
